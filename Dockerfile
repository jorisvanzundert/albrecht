...
RUN /bin/bash -l -c "gem install exifr --no-rdoc --no-ri"
...
RUN /bin/bash -l -c "sudo chmod a+w /home/theo/WebApps/albrecht/log;sudo chmod o+x /usr/local/bin/bootstrap_albrecht;sudo chmod o+x /usr/local/bin/pull_albrecht"
